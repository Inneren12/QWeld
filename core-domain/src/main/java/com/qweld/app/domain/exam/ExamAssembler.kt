...
class ExamAssembler(
    ...
    fun assemble(
        ...
        val allowFallback = when (mode) {
            ExamMode.IP_MOCK -> false
            ExamMode.PRACTICE, ExamMode.ADAPTIVE -> config.allowFallbackToEN
        }
        ...
        for (quota in blueprint.taskQuotas) {
            ...
            val selectionResult = when (mode) {
                ExamMode.IP_MOCK -> selectUniform(pool, stats, quota, seed, now, locale)
                ExamMode.PRACTICE, ExamMode.ADAPTIVE -> selectWeighted(pool, stats, quota, seed, now, locale)
            }
        ...
        val mutableStates = ordered.map { question ->
        ...
        val assembledQuestions = mutableStates.map { state ->
    ...
    private fun selectUniform(
        ...
        if (pool.size < quota.required) {
            return TaskSelectionResult(
                ...
                fresh = 0,
                reused = 0,
                deficitDetail = ExamAssemblyException.DeficitDetail(
                    taskId = quota.taskId,
                    need = quota.required,
                    have = pool.size,
    ...
    private fun selectWeighted(
        ...
        val entries: List<WeightedEntry<Question>> = sampler.order(pool) { question ->
            computeWeight(stats[question.id], config, now)
        }
