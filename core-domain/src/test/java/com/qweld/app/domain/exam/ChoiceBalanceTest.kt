...
class ChoiceBalanceTest {
    ...
    private val repo = FakeQuestionRepository(
        blueprint.taskQuotas.flatMap { quota ->
            generateQuestions(quota.taskId, quota.required * 2)
        }
    )
    private val assembler = ExamAssembler(
