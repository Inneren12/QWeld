...
class FakeQuestionRepository(
    ...
    override fun loadQuestions(
        ...
        val primary = questions.filter { it.taskId == taskId && it.locale.equals(locale, ignoreCase = true) }
        if (primary.isNotEmpty() || !allowFallbackToEnglish || locale.equals("EN", ignoreCase = true)) {
            return primary.sortedBy { it.id }
        }
        val fallback = questions.filter { it.taskId == taskId && it.locale.equals("EN", ignoreCase = true) }
...
class FakeUserStatsRepository(
    ...
    override fun loadQuestionStats(userId: String, questionIds: Collection<String>): Map<String, ItemStats> {
        return questionIds.mapNotNull { id -> stats[id]?.let { id to it } }.toMap()
    }
}

fun fixedClock(epochSeconds: Long = 0L): Clock = Clock.fixed(Instant.ofEpochSecond(epochSeconds), ZoneOffset.UTC)
...
fun buildQuestion(
    ...
    val choices = listOf("A", "B", "C", "D").mapIndexed { idx, label ->
