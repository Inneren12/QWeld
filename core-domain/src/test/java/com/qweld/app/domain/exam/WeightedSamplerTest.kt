...
class WeightedSamplerTest {
    ...
    fun `items with lower mastery appear more often`() {
        ...
        val stats = mapOf(
            "novice" to ItemStats("novice", attempts = 1, correct = 0, lastAnsweredAt = now.minusSeconds(86400)),
        ...
        repeat(500) { idx ->
            ...
            val order = sampler.order(listOf("novice", "master")) { id ->
                computeWeight(stats[id], config, now)
            }
